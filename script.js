...
/* CEN M04 Meaning/Panorama direct-link fix */
(function(){
  const hubMap = {
    '요단강 도하':'jordan',
    '여리고 함락':'jericho',
  ...
  function goExplore(btn){
    const group = btn.closest('.button-set');
    if(!group) return false;
    const eventName = group.dataset.event || '';
    const hub = group.dataset.hub || hubMap[eventName];
    if(!hub) return false;
    const txt = (btn.textContent || '').trim();
    if(txt.includes('Meaning') || txt.includes('연결탐험') || txt.includes('탐험')){
      location.href = `hubs/index.html?hub=${hub}#connections`;
      return true;
    }
    if(txt.includes('파노라마') || txt.includes('통합탐험')){
      location.href = `hubs/index.html?hub=${hub}#integration`;
      return true;
    }
    return false;
  }
  document.addEventListener('click', function(e){
    const btn = e.target.closest && e.target.closest('.button-set button, .button-set a');
    if(!btn) return;
    if(goExplore(btn)){
      e.preventDefault();
      e.stopPropagation();
